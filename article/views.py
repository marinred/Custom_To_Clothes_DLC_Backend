...
class CustomView(APIView):
    ...
    def post(self, request):
        ...
        base_image = Draft.objects.get(
            id=request.data["draft"]
        ).image.name  # draft image 이름
        style_image = style_image_url  # style image 이름

        os.system(
            "python style-transfer-pytorch/style_transfer/cli.py media/"
            + base_image
            + " "
            + style_image
            + " -s 156 -ii 30 -o media/temp/"
            + image_uuid
            + ".png"
    ...
    def put(self, request):
        ...
        if serializer_put.is_valid():
            serializer_put.save(
                image="result/" + image_uuid + ".png", style_id=style_id
            )
        else:
...
class RankArticleView(APIView):
    def get(self, request):
        article = Article.objects.annotate(like_count=Count("likes")).order_by(
            "-like_count"
        )
